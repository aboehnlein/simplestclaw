 ...
 *   OPENCLAW_AGENT_MODEL   - Model to use (e.g., anthropic/claude-haiku-4.5-20251001)
...
function validateEnvironment() {
  ...
  const configuredProviders = API_KEY_VARS.filter((key) => process.env[key]).map((key) =>
...
/**
 * Ensures ~/.openclaw/openclaw.json exists and configures the model.
...
function configureOpenClawModel() {
  ...
  if (fs.existsSync(configPath)) {
    try {
      ...
      console.warn(`Warning: Could not parse existing openclaw.json, will overwrite: ${err.message}`);
    }
  }
