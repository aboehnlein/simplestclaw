...
// Gateway URL Input Component
function GatewayUrlInput({
  value,
  onChange,
  placeholder,
}: {
  ...
  return (
    <div className="flex gap-3">
      ...
      />
      <Link
        href={value ? `/chat?gateway=${encodeURIComponent(value)}` : '#'}
...
// Local Flow Section Component
function LocalFlowSection({
  onBack,
  gatewayUrl,
  onGatewayUrlChange,
}: {
    ...
    <div className="mt-16">
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="mt-8 p-5 rounded-xl bg-white/[0.02] border border-white/5">
          ...
          <p className="text-[13px] text-emerald-400 mb-4">
            ...
            <li className="flex items-start gap-2">
              ...
              <div>
                ...
                <span className="text-white/40">
                  {' '}
                  Chat, see tool actions, and code execution in one place.
                </span>
              </div>
            </li>
            <li className="flex items-start gap-2">
...
const providerIcons = {
  anthropic: (
    <svg className="w-4 h-4 shrink-0" viewBox="0 0 24 24" fill="#D4A27F" aria-hidden="true">
  ...
  openai: (
    <svg className="w-4 h-4 shrink-0" viewBox="0 0 24 24" fill="white" aria-hidden="true">
  ...
  openrouter: (
    <svg className="w-4 h-4 shrink-0" viewBox="0 0 24 24" fill="#6366F1" aria-hidden="true">
      <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 17.93c-3.95-.49-7-3.85-7-7.93 0-.62.08-1.21.21-1.79L9 15v1c0 1.1.9 2 2 2v1.93zm6.9-2.54c-.26-.81-1-1.39-1.9-1.39h-1v-3c0-.55-.45-1-1-1H8v-2h2c.55 0 1-.45 1-1V7h2c1.1 0 2-.9 2-2v-.41c2.93 1.19 5 4.06 5 7.41 0 2.08-.8 3.97-2.1 5.39z" />
    </svg>
  ),
};

// Hosted Flow Section Component
function HostedFlowSection({
  onBack,
  gatewayUrl,
  onGatewayUrlChange,
  selectedProvider,
    ...
    <div className="mt-16">
      ...
      <div className="max-w-2xl mx-auto">
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              ...
              <ProviderButton
                ...
                onSelect={() => onSelectProvider('google')}
              />
              <ProviderButton
                provider="openrouter"
                label="OpenRouter"
        ...
        <p className="text-[12px] text-white/30 mt-3 text-center">
          {selectedProvider
            ? `You'll enter your ${getProviderDisplayName(selectedProvider)} API key on Railway.`
...
// Hosted Flow Info Component
function HostedFlowInfo() {
  return (
    <div className="mt-8 p-5 rounded-xl bg-white/[0.02] border border-white/5">
      ...
      <p className="text-[12px] text-white/30 mt-4 pt-4 border-t border-white/5">
        <span className="text-red-400">Note*</span> Requires Railway Hobby plan ($5/month). Free
        trial has memory limits.
      </p>
    </div>
  );
}
  ...
  const features = [
    {
      ...
      desc: 'Maintains context across all conversations and sessions.',
    },
    {
      title: 'Tool integrations',
      desc: 'Calendar, email, files, code execution, web search.',
    },
    {
      title: 'Privacy-first',
      ...
      <div className="max-w-[1400px] mx-auto">
        <div className="text-center mb-16">
          ...
          <p className="text-[17px] text-white/50 max-w-xl mx-auto">
            OpenClaw is an open-source AI that runs continuously, integrates with your tools, and
...
function ComparisonSection() {
  const traditionalSteps = [
    {
      step: 'Provision a virtual machine',
      sub: 'DigitalOcean, AWS, or similar. Need a credit card.',
    },
    {
      step: 'Configure SSH access',
      sub: 'Generate keys, add to server, test connection.',
    },
    { step: 'Install Node.js runtime', sub: 'Hope your package manager cooperates.' },
    {
      step: 'Clone and configure OpenClaw',
      sub: 'Environment variables, permissions, dependencies.',
    },
    {
      step: 'Set up Telegram bot via BotFather',
      sub: 'Create bot, get tokens, configure webhooks.',
    },
    ...
    <section id="compare" className="py-32 px-6 border-t border-white/5">
      <div className="max-w-[1400px] mx-auto">
        <div className="text-center mb-16">
          ...
          <p className="text-[17px] text-white/50 max-w-xl mx-auto">
            Traditional OpenClaw setup requires technical knowledge and takes about an hour. We
            reduced it to one click.
          </p>
        </div>
          ...
          <div className="p-8 rounded-2xl bg-white/[0.02] border border-white/5">
            ...
            <ol className="space-y-4">
              {traditionalSteps.map((item, index) => (
                <li key={`traditional-step-${item.step}`} className="flex items-start gap-4">
          ...
          <div className="p-8 rounded-2xl bg-emerald-500/[0.03] border border-emerald-500/10">
            ...
            <ol className="space-y-4">
              {simpleSteps.map((item, index) => (
                <li key={`simple-step-${item.step}`} className="flex items-start gap-4">
        ...
        <div className="max-w-[1400px] mx-auto">
          <div className="text-center">
            ...
            <p className="mt-6 text-[19px] text-white/50 leading-relaxed max-w-xl mx-auto">
              The easiest way to run OpenClaw. No terminal, no Telegram bots, no server
              configuration.
            </p>
          </div>

          {!mode && <ModeSelectionCards onSelectMode={setMode} />}
